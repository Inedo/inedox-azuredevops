...
internal sealed class AzureDevOpsClient : ILogSink
{
    ...
    public AzureDevOpsClient(GitServiceCredentials credentials, ILogSink? log = null) : this($"{credentials.ServiceUrl!.TrimEnd('/')}/", credentials.Password!, log)
    {
    }
    public AzureDevOpsClient(string url, SecureString token, ILogSink? log = null) : this(url, AH.Unprotect(token)!, log)
    {
    }
    public AzureDevOpsClient(string url, string token, ILogSink? log = null)
    {
    ...
    {
        this.LogDebug($"Requesting {typeof(TResult).Name} from {url}...");
